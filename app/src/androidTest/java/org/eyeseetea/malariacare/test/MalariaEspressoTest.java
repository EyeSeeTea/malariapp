...
public class MalariaEspressoTest {
    ...
    public static final String DATABASE_FULL_PATH =
            "/data/data/org.eyeseetea.malariacare/databases/" + DATABASE_NAME;
    ...
    public static void cleanDB() {
        ...
        Delete.tables(Question.class, CompositeScore.class, Option.class, Answer.class,
                ...
                Survey.class);
        /*new Delete().from(Question.class).where(Condition.column(Question$Table.ID).isNotNull()
        ).query();
        new Delete().from(CompositeScore.class).where(Condition.column(CompositeScore$Table.ID)
        .isNotNull()).query();;
        new Delete().from(Option.class).where(Condition.column(Option$Table.ID).isNotNull())
        .query();;
        new Delete().from(Answer.class).where(Condition.column(Answer$Table.ID).isNotNull())
        .query();;
        new Delete().from(Header.class).where(Condition.column(Header$Table.ID).isNotNull())
        .query();;
        new Delete().from(Tab.class).where(Condition.column(Tab$Table.ID).isNotNull()).query();;
        new Delete().from(Program.class).where(Condition.column(Program$Table.ID).isNotNull())
        .query();;
        new Delete().from(OrgUnit.class).where(Condition.column(OrgUnit$Table.ID).isNotNull())
        .query();;
        ...
        new Delete().from(Survey.class).where(Condition.column(Survey$Table.ID).isNotNull())
        .query();;*/
    }
    ...
    public static boolean databaseExists() {
        ...
        try {
            checkDB = SQLiteDatabase.openDatabase(DATABASE_FULL_PATH, null,
    ...
    public static List<Survey> mockSurveys(int numOrgs, int numPrograms) {
        ...
        List<Survey> surveys = new Select().from(Survey.class).where(
                Condition.column(Survey$Table.ID_USER).eq(user.getId_user())).queryList();
        return surveys;
    }
