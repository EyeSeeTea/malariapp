...
import static org.eyeseetea.malariacare.test.utils.SDKTestUtils.goToSentSurveys;
...
public class DashboardAdaptersNumberSentRowsTest {
    ...
    @Test
    public void sentTabShowsExpectedSurveys(){
        //GIVEN
        login(HNQIS_DEV_CI, TEST_USERNAME_WITH_PERMISSION, TEST_PASSWORD_WITH_PERMISSION);
        waitForPull(DEFAULT_WAIT_FOR_PULL);
        int expectedPrograms=(int)Program.count();
        int expectedOrgUnits=(int)OrgUnit.count();
        int expectedSurveysInAdapter = expectedOrgUnits*expectedPrograms;
        //WHEN
        goToSentSurveys();

        //THEN
        onView(allOf(withId(android.R.id.list), withChild(withId(R.id.assessment_row)))).check(matches(withListSize(expectedSurveysInAdapter)));
    }
