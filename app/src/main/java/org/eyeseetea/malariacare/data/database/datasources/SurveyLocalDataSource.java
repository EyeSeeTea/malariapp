...
public class SurveyLocalDataSource implements ISurveyDataSource, IDataLocalDataSource {
    ...
    public List<Survey> getSurveys(SurveyFilter filter) {
        ...
        if(!hasOrgUnitAndProgram(filter) && !filter.isLastSurvey()) {
            surveyDBS = getSurveysDB(filter.getSurveyStatus());

        }
        if(hasOrgUnitAndProgram(filter) && filter.isLastSurvey()){
            SurveyDB surveyDB = SurveyDB.findPlannedByOrgUnitAndProgram(filter.getOrgUnitUId(),
                    filter.getProgramUId());
            if(surveyDB!=null) {
                surveyDBS.add(surveyDB);
            }
        }
        if(filter.getOrgUnitUId() == null && filter.getProgramUId() == null && filter.isLastSurvey()){
    ...
    private boolean hasOrgUnitAndProgram(SurveyFilter filter) {
        return filter.getOrgUnitUId() != null && filter.getProgramUId() != null;
    }
    ...
    public List<? extends IData> getDataToSync() throws Exception {
        List<Survey> surveys = getSurveys(new SurveyFilter(SurveyStatus.COMPLETED));
        return surveys;
    }
